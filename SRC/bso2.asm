...
USB_CTRL_PORT           EQU         $7FE0 ; FTDI245 VIA CTRL (PWREN#)
USB_PWREN_M             EQU         $20   ; 0 = USB configured/ready
...
SREC_GO_ADDR:           DS          2   ; LAST S7/S8/S9 ENTRY ADDR FROM L S
SREC_GO_VALID:          DS          1   ; 1 IF SREC_GO_ADDR IS VALID
...
ACIA_DBG_LAST_STATUS:   DS          1   ; BOOT DEBUG: LAST PRINTED ACIA STATUS
...
DBG_STACK_IMAGE         EQU         $0600 ; SAVED PAGE-1 TAIL FOR DEBUG RESUME
...
?COOKIE_OK:
        ...
        ; VECTOR TRAMPOLINES/HANDLERS; FUTURE TRAMPOLINES MAY BEHAVE DIFFERENTLY.
                        LDA         #SYSF_RESET_FLAG_M
                        TSB         SYS_FLAGS
                        LDA         #<MSG_RESET_TRIGGERED ; RESET BUTTON PATH
                        LDX         #>MSG_RESET_TRIGGERED
...
?BOOT_GO_MONITOR:
                        STZ         REC_CMD_ACTIVE
                        LDA         #<MSG_MONITORSTART
                        LDX         #>MSG_MONITORSTART
                        JSR         BOOT_PRT_LABEL_FANOUT
                        JMP         MONITOR_CLEAN
...
; ----------------------------------------------------------------------------
; SUBROUTINE: USB_IS_READY
; DESCRIPTION: REPORTS FTDI USB LINK STATE FROM PWREN# (ACTIVE LOW)
; INPUT: NONE
; OUTPUT: C=0 IF READY, C=1 IF NOT READY
; ----------------------------------------------------------------------------
USB_IS_READY:
                        LDA         #USB_PWREN_M
                        BIT         USB_CTRL_PORT
...
; ----------------------------------------------------------------------------
; SUBROUTINE: BOOT_WRITE_BYTE_FANOUT
; DESCRIPTION: BOOT PROMPT OUTPUT:
;              - USB SERIAL ONLY WHEN USB_IS_READY
;              - ACIA ALWAYS (RESET + REINIT BEFORE EACH BYTE WRITE)
; INPUT: A = BYTE TO OUTPUT
; OUTPUT: BYTE EMITTED PER ABOVE FANOUT POLICY
; ----------------------------------------------------------------------------
BOOT_WRITE_BYTE_FANOUT:
                        PHA
                        JSR         USB_IS_READY
                        BCS         ?BWBF_SKIP_USB
                        PLA
                        ...
                        JSR         WRITE_BYTE
?BWBF_SKIP_USB:
                        JSR         ACIA_INIT_SERIAL ; includes ACIA program reset
                        PLA
                        JSR         ACIA_WRITE_BYTE  ; best effort (ignore timeout)
                        RTS

; ----------------------------------------------------------------------------
; SUBROUTINE: BOOT_WRITE_BYTE_FANOUT_LIVE_ACIA
; DESCRIPTION: BOOT OUTPUT WITHOUT ACIA PROGRAM RESET/REINIT
; INPUT: A = BYTE TO OUTPUT
; OUTPUT: BYTE EMITTED TO USB(WHEN READY) + ACIA BEST-EFFORT
; NOTES:
;   - USE FOR BOOT INPUT LOOPS THAT ALSO POLL ACIA RX.
; ----------------------------------------------------------------------------
BOOT_WRITE_BYTE_FANOUT_LIVE_ACIA:
                        PHA
                        JSR         USB_IS_READY
                        BCS         ?BWBFLA_SKIP_USB
                        PLA
...
; ----------------------------------------------------------------------------
; SUBROUTINE: BOOT_PRT_CSTRING_FANOUT
; DESCRIPTION: PRINTS NUL-TERMINATED STRING AT STR_PTR VIA BOOT_WRITE_BYTE_FANOUT
; INPUT: STR_PTR = SOURCE POINTER
; OUTPUT: STRING EMITTED USING BOOT PROMPT FANOUT POLICY
; ----------------------------------------------------------------------------
BOOT_PRT_CSTRING_FANOUT:
?BPCS_LOOP:
                        LDY         #$00
                        LDA         (STR_PTR),Y
                        BEQ         ?BPCS_DONE
                        JSR         BOOT_WRITE_BYTE_FANOUT
                        INC         STR_PTR
                        BNE         ?BPCS_LOOP
                        INC         STR_PTR+1
                        BRA         ?BPCS_LOOP
...
; ----------------------------------------------------------------------------
; SUBROUTINE: BOOT_PRT_LABEL_FANOUT
; DESCRIPTION: PRINTS STRING AT AX USING BOOT PROMPT FANOUT POLICY
; INPUT: A = LOW BYTE OF STRING ADDRESS, X = HIGH BYTE
; ----------------------------------------------------------------------------
BOOT_PRT_LABEL_FANOUT:
                        STA         STR_PTR
                        STX         STR_PTR+1
                        JMP         BOOT_PRT_CSTRING_FANOUT
...
; ----------------------------------------------------------------------------
; SUBROUTINE: ACIA_GET_BYTE_NB
; DESCRIPTION: NON-BLOCKING ACIA BYTE FETCH (POLLED)
; INPUT: NONE
; OUTPUT: ACC = BYTE RECEIVED IF AVAILABLE, ELSE $00
; FLAGS: CARRY CLEAR IF BYTE RETURNED, SET IF RX BUFFER EMPTY
; ZP USED: NONE
; ----------------------------------------------------------------------------
ACIA_GET_BYTE_NB:
                        JSR         ACIA_CHECK_BYTE
...
; ----------------------------------------------------------------------------
; SUBROUTINE: ACIA_READ_BYTE_NB
; DESCRIPTION: COMPATIBILITY ALIAS FOR ACIA_GET_BYTE_NB
; INPUT: NONE
; OUTPUT: ACC = BYTE RECEIVED IF AVAILABLE, ELSE $00
; FLAGS: CARRY CLEAR IF BYTE RETURNED, SET IF RX BUFFER EMPTY
; ZP USED: NONE
; ----------------------------------------------------------------------------
ACIA_READ_BYTE_NB:
                        JMP         ACIA_GET_BYTE_NB

; ----------------------------------------------------------------------------
; SUBROUTINE: ACIA_GET_BYTE_BL
; DESCRIPTION: BLOCKING ACIA BYTE FETCH (SPINS ON ACIA_GET_BYTE_NB)
; INPUT: NONE
; OUTPUT: ACC = BYTE RECEIVED
; FLAGS: CARRY CLEAR ON RETURN
; ZP USED: NONE
; ----------------------------------------------------------------------------
ACIA_GET_BYTE_BL:
?AGBB_WAIT:
                        JSR         ACIA_GET_BYTE_NB
                        BCS         ?AGBB_WAIT
                        RTS

; ----------------------------------------------------------------------------
; SUBROUTINE: ACIA_READ_BYTE_BL
; DESCRIPTION: COMPATIBILITY ALIAS FOR ACIA_GET_BYTE_BL
; INPUT: NONE
; OUTPUT: ACC = BYTE RECEIVED
; FLAGS: CARRY CLEAR ON RETURN
; ZP USED: NONE
...
; DESCRIPTION: READS CHAR FROM ACTIVE SERIAL INPUT (FTDI OR ACIA, BLOCKING)
; INPUT: NONE
...
; ----------------------------------------------------------------------------
; SUBROUTINE: BOOT_READ_BYTE_ECHO_UPPER_FANOUT
; DESCRIPTION: READS ONE BYTE, UPPERCASES, ECHOES VIA BOOT PROMPT FANOUT
; INPUT: NONE
; OUTPUT: A = UPPERCASED BYTE
; ----------------------------------------------------------------------------
BOOT_READ_BYTE_ECHO_UPPER_FANOUT:
                        JSR         READ_BYTE
                        JSR         UTIL_TO_UPPER
                        PHA
                        JSR         BOOT_WRITE_BYTE_FANOUT
...
; ----------------------------------------------------------------------------
; SUBROUTINE: BOOT_READ_CWM_KEY_NB
; DESCRIPTION: NON-BLOCKING C/W/M KEY FETCH FROM FTDI OR ACIA
; INPUT: NONE
; OUTPUT: C=0 WITH A=UPPERCASED KEY (ECHOED VIA BOOT FANOUT), C=1 IF NO KEY
; NOTES:
;   - FTDI IS POLLED FIRST (ONLY WHEN USB_IS_READY).
;   - ACIA_READ_BYTE_NB IS POLLED AFTER FTDI CHECK OR IMMEDIATELY WHEN USB
;     IS NOT READY.
; ----------------------------------------------------------------------------
BOOT_READ_CWM_KEY_NB:
                        PUSH        X, Y
                        ; Poll ACIA first so FTDI status behavior cannot starve
                        ; ACIA C/W/M capture during boot prompt windows.
                        JSR         ACIA_READ_BYTE_NB
                        BCC         ?BRCN_HAVE_ACIA
                        JSR         USB_IS_READY
                        BCS         ?BRCN_NONE
                        JSR         WDC_CHECK_BYTE
                        ...
                        JSR         UTIL_TO_UPPER
                        PHA
                        JSR         BOOT_WRITE_BYTE_FANOUT_LIVE_ACIA
                        PLA
                        CLC
                        PULL        Y, X
                        RTS
...
; ----------------------------------------------------------------------------
; SUBROUTINE: BOOT_WAIT_CWM_POLL_333MS_NB
; DESCRIPTION: ~333MS WAIT SLICE WITH CONTINUOUS FTDI/ACIA CWM KEY POLLING
; INPUT: NONE
; OUTPUT: C=0 WITH A=UPPERCASED KEY, C=1 IF NO KEY IN THIS SLICE
; ----------------------------------------------------------------------------
BOOT_WAIT_CWM_POLL_333MS_NB:
                        PUSH        X, Y
                        LDX         #$0F ; MATCH DELAY_333MS CALIBRATION (15 * DELAY)
?BWCP333_LOOP:
                        JSR         DELAY
                        JSR         BOOT_READ_CWM_KEY_NB
                        BCC         ?BWCP333_HAVE
                        DEX
                        BNE         ?BWCP333_LOOP
                        PULL        Y, X
...
; ----------------------------------------------------------------------------
; SUBROUTINE: BOOT_HEX_NIBBLE_TO_ASCII
; DESCRIPTION: CONVERTS LOW NIBBLE IN A (0..15) TO ASCII HEX ('0'..'F')
; INPUT: A = VALUE (LOW NIBBLE USED)
; OUTPUT: A = ASCII HEX DIGIT
; ----------------------------------------------------------------------------
BOOT_HEX_NIBBLE_TO_ASCII:
                        AND         #$0F
                        CMP         #$0A
                        BCC         ?BHNTA_DIG
                        CLC
...
; ----------------------------------------------------------------------------
; SUBROUTINE: BOOT_WRITE_HEX_A_LIVE_ACIA
; DESCRIPTION: WRITES A AS TWO HEX CHARS USING LIVE-ACIA BOOT FANOUT
; INPUT: A = BYTE
; OUTPUT: NONE
; ----------------------------------------------------------------------------
BOOT_WRITE_HEX_A_LIVE_ACIA:
                        PHA
                        LSR
                        LSR
                        LSR
                        LSR
                        JSR         BOOT_HEX_NIBBLE_TO_ASCII
                        JSR         BOOT_WRITE_BYTE_FANOUT_LIVE_ACIA
                        PLA
                        JSR         BOOT_HEX_NIBBLE_TO_ASCII
                        JSR         BOOT_WRITE_BYTE_FANOUT_LIVE_ACIA
...
; ----------------------------------------------------------------------------
; SUBROUTINE: BOOT_ACIA_DEBUG_STATUS_NB
; DESCRIPTION: DEBUG TRACE (STATUS CHANGES ONLY): PRINTS " [AS=HH]"
; INPUT: NONE
; OUTPUT: NONE
; NOTES:
;   - TEMP INSTRUMENTATION FOR BOOT C/W/M RX DIAGNOSTICS.
; ----------------------------------------------------------------------------
BOOT_ACIA_DEBUG_STATUS_NB:
                        PHA
                        LDA         ACIA_STATUS
                        CMP         ACIA_DBG_LAST_STATUS
                        BEQ         ?BADSN_DONE
                        STA         ACIA_DBG_LAST_STATUS
                        PHA
                        LDA         #' '
                        JSR         BOOT_WRITE_BYTE_FANOUT_LIVE_ACIA
                        LDA         #'['
                        ...
                        JSR         BOOT_WRITE_BYTE_FANOUT_LIVE_ACIA
                        PLA
                        JSR         BOOT_WRITE_HEX_A_LIVE_ACIA
                        LDA         #']'
                        JSR         BOOT_WRITE_BYTE_FANOUT_LIVE_ACIA
?BADSN_DONE:
                        PLA
...
; ----------------------------------------------------------------------------
BOOT_WAIT_CWM_KEY:
                        JSR         ACIA_INIT_SERIAL ; keep ACIA live through C/W/M poll loop
                        LDA         #$FF ; force first debug emission
                        STA         ACIA_DBG_LAST_STATUS
                        LDA         #$06
                        STA         TERM_WAIT_SECS
                        LDA         #$FF
...
?BWCK_RST_WAIT_1S:
                        ...
                        EOR         #$FF
                        STA         TERM_WAIT_LED
                        JSR         BOOT_WAIT_CWM_POLL_333MS_NB
                        BCC         ?BWCK_RST_HAVE
                        DEY
...
; ----------------------------------------------------------------------------
BOOT_WAIT_CM_KEY:
                        JSR         ACIA_INIT_SERIAL ; keep ACIA live through C/M poll loop
                        LDA         #$FF ; force first debug emission
                        STA         ACIA_DBG_LAST_STATUS
                        LDA         #$06
                        STA         TERM_WAIT_SECS
...
; ----------------------------------------------------------------------------
; SUBROUTINE: BOOT_ACIA_READ_BYTE_ECHO_NB
; DESCRIPTION: NON-BLOCKING ACIA READ + ECHO DURING BOOT WAIT POLLS
; INPUT: NONE
; OUTPUT: NONE
; NOTES:
;   - IF NO ACIA BYTE IS PENDING, RETURNS IMMEDIATELY.
;   - IF A BYTE IS PRESENT, ECHOES IT VIA BOOT FANOUT.
;   - DOES NOT PARTICIPATE IN C/W/M DECISION LOGIC.
; ----------------------------------------------------------------------------
BOOT_ACIA_READ_BYTE_ECHO_NB:
                        JSR         ACIA_READ_BYTE_NB
                        BCS         ?BARBEN_DONE
                        JSR         BOOT_WRITE_BYTE_FANOUT
?BARBEN_DONE:
...
;                IF START OMITTED, FALL BACK TO LAST L S GO ADDR, THEN L S FIRST
...
; ----------------------------------------------------------------------------
; SUBROUTINE: STEP_OPCODE_SUPPORTED
; DESCRIPTION: C=0 IF OPCODE IN A IS SAFE FOR LINEAR N-STEPPING
;              C=1 FOR NON-LINEAR CONTROL FLOW THAT N MUST REJECT
; NOTES:
;   - JSR ($20) IS ALLOWED: N TREATS IT AS STEP-OVER BY PATCHING THE
;     SEQUENTIAL RETURN-SITE ADDRESS.
;   - CONDITIONAL/UNCONDITIONAL BRANCHES, BBR/BBS, JMP, RTS, AND RTI ARE
;     REJECTED FOR NOW.
; ----------------------------------------------------------------------------
STEP_OPCODE_SUPPORTED:
                        CMP         #$40 ; RTI
                        BEQ         ?SOS_NO
                        CMP         #$60 ; RTS
                        BEQ         ?SOS_NO
...
?LGS_GO:
                        ...
                        LDA         #$01
                        STA         REC_RUN_VALID
                        STZ         BRK_FLAG ; AUTO-GO IS EXPLICIT RUN; DROP OLD CTX
...
; ----------------------------------------------------------------------------
CMD_DO_LOAD_SREC:
                        STZ         BRK_FLAG ; LOAD/RELOAD INVALIDATES OLD DEBUG CTX
                        STZ         SREC_FIRST_VALID
                        STZ         SREC_FIRST_ADDR
                        STZ         SREC_FIRST_ADDR+1
                        STZ         SREC_GO_VALID
...
DBG_RESUME_CONTEXT:
                        ...
                        INX
?DRC_RESTORE_LOOP:
                        LDA         DBG_STACK_IMAGE,X
                        STA         $0100,X
                        INX
                        BNE         ?DRC_RESTORE_LOOP
?DRC_RTI_FRAME:
                        LDA         DBG_SP
                        TAX
                        TXS
...
; ----------------------------------------------------------------------------
; SUBROUTINE: DEBUG_SNAPSHOT_STACK_TAIL
; DESCRIPTION: SAVES PAGE-1 BYTES ABOVE DBG_SP SO MONITOR STACK TRAFFIC DOES
;              NOT DESTROY THE SUSPENDED RETURN CHAIN NEEDED BY R/RTI RESUME.
; INPUT: DBG_SP = PRE-DEBUG STACK POINTER
; OUTPUT: DBG_STACK_IMAGE[DBG_SP+1..$FF] = PAGE1 SNAPSHOT
; ----------------------------------------------------------------------------
DEBUG_SNAPSHOT_STACK_TAIL:
                        LDX         DBG_SP
                        CPX         #$FF
                        BEQ         ?DSST_DONE
                        INX
...
; ----------------------------------------------------------------------------
; SUBROUTINE: DEBUG_IS_SYNTH_STEP_HIT
; DESCRIPTION: C=1 IF CURRENT BRK CONTEXT IS AN N-COMMAND SYNTHETIC STEP HIT
; OUTPUT: C=1 SYNTHETIC STEP HIT, C=0 OTHERWISE
; ----------------------------------------------------------------------------
DEBUG_IS_SYNTH_STEP_HIT:
                        LDA         STEP_ACTIVE
                        BEQ         ?DISH_NO
                        LDA         DBG_MODE
                        CMP         #DBG_MODE_BRK
                        ...
                        BNE         ?DISH_NO
                        LDA         STEP_ORIG
                        BEQ         ?DISH_NO ; STEPPED-TO REAL BRK, NOT SYNTHETIC
                        SEC
                        RTS
?DISH_NO:
                        CLC
...
; ----------------------------------------------------------------------------
; SUBROUTINE: DEBUG_PRINT_STOP_LINE
; DESCRIPTION: PRINTS STOP HEADER + PC FOR JSR/NMI/IRQ/BRK/STEP
; FORMAT: "stop: JSR  pc:HHLL", "stop: NMI  pc:HHLL", "stop: IRQ  pc:HHLL",
;         "stop: STEP  pc:HHLL", OR "stop: BRK XX  pc:HHLL"
; ----------------------------------------------------------------------------
DEBUG_PRINT_STOP_LINE:
                        PUSH        A, X, Y
                        JSR         PRT_CRLF
                        PRT_CSTRING STR_STOP
                        LDA         DBG_MODE
                        CMP         #DBG_MODE_BRK
                        BEQ         ?DPSL_BRK
                        CMP         #DBG_MODE_NMI
                        ...
                        BRA         ?DPSL_PC
?DPSL_NMI:
                        PRT_CSTRING STR_NMI_LOWER
                        BRA         ?DPSL_PC
?DPSL_IRQ:
                        PRT_CSTRING STR_IRQ_LOWER
                        BRA         ?DPSL_PC
?DPSL_BRK:
                        JSR         DEBUG_IS_SYNTH_STEP_HIT
                        BCC         ?DPSL_BRK_REAL
                        PRT_CSTRING STR_STEP
                        BRA         ?DPSL_PC
?DPSL_BRK_REAL:
                        PRT_CSTRING STR_BRK
                        LDA         DBG_BRK_SIG
...
; ----------------------------------------------------------------------------
; SUBROUTINE: DEBUG_PRINT_REGS
; DESCRIPTION: PRINTS REGISTERS/FLAGS FOR ALL DEBUG STOPS
; FORMAT: "regs: A=HH X=HH Y=HH P=HH S=HH [flags]"
; ----------------------------------------------------------------------------
DEBUG_PRINT_REGS:
                        PUSH        A, X, Y
                        JSR         PRT_CRLF
                        PRT_CSTRING STR_REGS
...
DEBUG_PRINT_CONTEXT:
                        ...
                        BRA         ?DPC_REGS
?DPC_STD_NEXT:
                        JSR         DEBUG_PRINT_NEXT_LINE
?DPC_REGS:
                        JSR         DEBUG_PRINT_REGS
                        RTS

; ----------------------------------------------------------------------------
; SUBROUTINE: DEBUG_PRINT_NEXT_LINE
; DESCRIPTION: PRINTS "next: " + DISASSEMBLY AT DBG_PC
; ----------------------------------------------------------------------------
DEBUG_PRINT_NEXT_LINE:
                        PUSH        A, X, Y
                        JSR         PRT_CRLF
                        PRT_CSTRING STR_NEXT_LOWER
                        LDA         DBG_PC_LO
                        STA         PTR_DUMP_CUR
                        LDA         DBG_PC_HI
                        STA         PTR_DUMP_CUR+1
...
; DESCRIPTION: PRINTS BRK CONTEXT CURR/NEXT ON SEPARATE LINES.
;              DEFAULT: CURR=(DBG_PC-2), NEXT=DBG_PC.
;              SYNTHETIC N-STEP HIT: CURR=STEP_ADDR, NEXT=STEP_ADDR+LEN(STEP_ORIG).
; ----------------------------------------------------------------------------
DEBUG_PRINT_BRK_CURR_NEXT:
                        PUSH        A, X, Y
                        JSR         PRT_CRLF
                        PRT_CSTRING STR_CURR_LOWER
                        JSR         DEBUG_IS_SYNTH_STEP_HIT
                        BCC         ?DBCN_DEFAULT
                        LDA         STEP_ADDR
                        STA         PTR_DUMP_CUR
                        ...
                        LDA         STEP_ORIG
                        TAX
                        LDA         U_OP_LEN_TAB,X ; NEXT = STEP_ADDR + LEN(ORIG OPCODE)
                        CLC
                        ADC         STEP_ADDR
                        STA         PTR_TEMP
                        LDA         STEP_ADDR+1
...
?SNMI_MONITOR_IDLE:
                        ; Monitor-side NMI is informational only; do not allow R
                        ; to resume into monitor/parser internals.
                        STZ         BRK_FLAG
...
; DESCRIPTION: CHECKS SERIAL INPUT STATUS (ACIA FIRST, THEN FTDI WHEN READY)
; INPUT: NONE
; OUTPUT: ACC = STATUS
...
MSG_I_INFO:             DB          $0D, $0A, "R0M0V2I05", 0
...
MSG_N_CTRL:             DB          $0D, $0A, "N UNSUPPORTED FOR BRANCH/JUMP/RTI/RTS", 0
...
STR_STOP:               DB          "stop: ", 0
STR_JSR:                DB          "JSR", 0
STR_NMI_LOWER:          DB          "NMI", 0
STR_IRQ_LOWER:          DB          "IRQ", 0
STR_STEP:               DB          "STEP", 0
STR_BRK:                DB          "BRK ", 0
STR_PC_LOWER:           DB          "pc:", 0
STR_CURR_LOWER:         DB          "curr: ", 0
STR_NEXT_LOWER:         DB          "next: ", 0
STR_REGS:               DB          "regs: ", 0
KDATA_REGION_END:
                        END
