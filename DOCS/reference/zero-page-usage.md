...
| HEARTBEAT_PHASE | 1 byte | `$7C` | Timer1 heartbeat overlay phase (`00/FF`) for visible LED patterns | |
| HEARTBEAT_DIV | 1 byte | `$7D` | Timer1 heartbeat software divider / pattern cadence scratch | |
| TERM_CUR_COL | 1 byte | `$7E` | Current output column tracker (terminal wrap state) | |
| TERM_WAIT_LED | alias | `$7E` | Legacy alias for width-prompt LED scratch; aliases `TERM_CUR_COL` | |
...
| `I T0` | Heartbeat mode/status control | 7, 8 |
