...
- Board: `16x16`, toroidal wraparound
- Build target: `make -C SRC life`
- Link/load address: `$5000`
- Output artifact: `SRC/life.s19`
...
- `B`: seed blinker
- `R`: randomize board
- `P`: enter pattern mode and type 16 rows using `.` and `#`
- `Q`: break/exit path
...
- Pattern mode prompts as `ROW hh > `
- Short pattern rows are padded with `.`
- `Q` aborts pattern entry and returns to the main loop
...
- Startup banner text is `=== LIFE 16x16 ===`.
