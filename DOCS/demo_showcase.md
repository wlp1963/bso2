...
Note: this is a historical capture. Current monitor behavior may differ (for example: `T` now supports `C|20|40|80|132`, protected range is `$0000-$0FFF`, command-mode `W` is removed, game prompt text includes `(@$78)`, `R START` in active context now hints `R CTX ACTIVE, R [A/X/Y=] or !R START`, and debug stops now use the newer `stop:` / `curr:` / `next:` / `regs:` format instead of the older `CURR:` / `STATE:` transcript lines shown here).
