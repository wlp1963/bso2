...
static void usage(void) {
    fprintf(stderr,
            ...
            "  ls-send S19 [--ready-timeout S] [--done-timeout S] [--no-wait-done]\n"
            "  lgs-send S19 [--ready-timeout S] [--done-timeout S] [--no-wait-done]\n"
...
static int cmd_ls_send_common(int argc,
    ...
    if (argc < 1) {
        fprintf(stderr, "error: %s requires S19 file\n", auto_go ? "lgs-send" : "ls-send");
        return 2;
    }
    srec_file = argv[0];
