...
Current release marker: `R0M0V2I05` (`I` = `INTERNAL`)
...
- Debug stop output uses `stop` / `next` / `regs` lines for `JSR` / `NMI` / `IRQ`, with `BRK` / `STEP` adding a `curr` line and explicit `STEP` vs `BRK xx` stop type
...
tools/bso2com/bso2com ls-send game.s19
tools/bso2com/bso2com lgs-send game.s19
```
